...
def post_home():
    ...
    if user_type == 'student':
        ...
        if company:
            ...
            job_count = get_record('SELECT COUNT(*) as count FROM jobs WHERE CompanyID = %s AND (DeadlineDate IS NULL OR DeadlineDate >= CURDATE())', (company_id,))
            stats['active_jobs'] = job_count['count'] if job_count else 0
            
            # Recent job applications to company's jobs
            raw_activities = get_records('''
                SELECT ja.ApplicationDate, ast.Name AS Status, j.Title AS JobTitle, u.First, u.Last
            ...
            activities = []
            for activity in raw_activities:
                description = f"{activity['First']} {activity['Last']} applied to '{activity['JobTitle']}' - {activity['Status']}"
                activities.append({
            ...
            raw_upcoming_items = get_records('''
                ...
                FROM jobs j
                WHERE CompanyID = %s AND DeadlineDate >= CURDATE()
            ...
            for item in raw_upcoming_items:
                ...
                upcoming_items.append({
                    'title': item['Title'],
                    'subtitle': f"{item['ApplicationCount']} applications",
                    'date': deadline.strftime('%Y-%m-%d') if deadline else 'No deadline',
                    'days_left': days_left
                })
    ...
    featured_content = []
    if user_type == 'student':
        # Example: Recommend jobs based on latest postings
        ...
        pass  # Future: could feature courses or teaching resources
    elif user_type == 'company':
        pass  # Future: could feature student profiles or analytics
